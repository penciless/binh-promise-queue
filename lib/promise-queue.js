...
function PromiseQueue(name) {
    ...
    queue.interface = function() {
        var object, source, name, args = arguments;

        if (args.length === 1 && typeof args[0] === 'string') {
            name = args[0];
            return interfaces[name] || interfacess[name] || {};
        }
        ...
        var clone = object instanceof Function ? function() {
            return queue.add.apply(queue, [source || null, object].concat(Array.from(arguments)));
        } : {};

        if (clone instanceof Function) {
            clone.args = function(argsCallback) {
                return queue.addAsync.apply(queue, [source || null, object, argsCallback]);
            };
        }
